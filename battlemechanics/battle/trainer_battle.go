...
func (t *TrainerBattle) handleSwitch(switchAction data.BattleAction, trainer battletrainer.BattleTrainer) {
	t.log(fmt.Sprintf("%s is switching %s for %s", t.getTrainerName(trainer.GetTrainer()),
...
func (t *TrainerBattle) performAttack(attackAction data.BattleAction, trainer battletrainer.BattleTrainer, target battletrainer.BattleTrainer) int {
	...
	if effect != data.NOR {
		if effect == data.MNE || effect == data.NVR {
			t.log("Not very effective!")
		} else if effect == data.SUP || effect == data.HYP {
			t.log("Super effective!")
		} else {
			t.log("This move has No effect to the target!")
		}
	}
...
// log formatters
func (t *TrainerBattle) getTrainerName(trainer *data.BaseTrainer) string {
	switch trainer.Name {
	case t.user.GetTrainer().Name:
		return t.getUserName()
	case t.opponent.GetTrainer().Name:
		return t.getOpponenetName()
	}
