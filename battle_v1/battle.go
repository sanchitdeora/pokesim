...
func getPokemonAttackOrder(userActivePokemon *data.BattlePokemon, trainerActivePokemon *data.BattlePokemon, userInputChan <-chan *data.BattleAction) (inputs []*data.BattleAction) {
	...
	if userInput.Type == data.Attack && trainerInput.Type == data.Attack {
...
func switchPokemonWithIndex(switchingPokemonIndex int, activePokemon *data.BattlePokemon, inBattleParty *[]*data.BattlePokemon) *data.BattlePokemon {
...
func calculateAttackDamage(attackPokemon *data.BattlePokemon, targetPokemon *data.BattlePokemon, attackMove *data.Moves, battleTypeAttackCoeff float64) int {
